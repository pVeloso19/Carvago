...
export default {
  ...
  setup () {
    ...
    const marcaRoute = computed(() => {
      return self.$route.params.marca
    })

    const modeloRoute = computed(() => {
      return self.$route.params.modelo
    })
  ...
  watch: {
    modeloRoute (newValue, oldValue) {
      getCarros()
    },
    marcaRoute (newValue, oldValue) {
      getCarros()
    }
  }
}
